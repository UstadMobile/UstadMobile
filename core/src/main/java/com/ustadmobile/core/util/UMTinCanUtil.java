...
public class UMTinCanUtil {
    ...
    public static long parse8601Duration(String duration) {

        try {
            Duration time = DatatypeFactory.newInstance().newDuration(duration);
            return time.getTimeInMillis(Calendar.getInstance());
        } catch (DatatypeConfigurationException | IllegalArgumentException e) {

            try {
                if (duration.contains("S")) {
                    long seconds = (long) Double.parseDouble(duration.substring(duration.indexOf("P") + 1, duration.indexOf("S")));
                    return seconds * 1000;
                } else if (duration.contains("W")) {
                    long weeks = (long) Double.parseDouble(duration.substring(duration.indexOf("P") + 1, duration.indexOf("W")));
