...
class EpubNavDocument {
     ...
     * Table of nav tags
     */
    private val navItems: MutableMap<String, EpubNavItem>

    /**
     * Vector of all navigation elements found
     */
    private val navElements: MutableList<EpubNavItem>
    ...
    val toc: EpubNavItem?
        get() {
            for (item in navElements) {
                ...
                if (Arrays.asList(*item.navElEpubTypeAttr!!.split("\\s+".toRegex()).dropLastWhile { it.isEmpty() }.toTypedArray())
