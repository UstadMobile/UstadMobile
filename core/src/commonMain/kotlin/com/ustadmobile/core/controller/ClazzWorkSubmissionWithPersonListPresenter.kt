...
class ClazzWorkSubmissionWithPersonListPresenter(context: Any, arguments: Map<String, String>, view: ClazzWorkSubmissionWithPersonListView,
                          lifecycleOwner: DoorLifecycleOwner, systemImpl: UstadMobileSystemImpl,
                          db: UmAppDatabase, repo: UmAppDatabase,
                          activeAccount: DoorLiveData<UmAccount?>)
    : UstadListPresenter<ClazzWorkSubmissionWithPersonListView, ClazzWorkSubmissionWithPerson>(context, arguments, view, lifecycleOwner, systemImpl,
    ...
    class ClazzWorkSubmissionWithPersonListSortOption(val sortOrder: SortOrder, context: Any) : MessageIdOption(sortOrder.messageId, context)

    override fun onCreate(savedState: Map<String, String>?) {
        super.onCreate(savedState)
        updateListOnView()
        view.sortOptions = SortOrder.values().toList().map { ClazzWorkSubmissionWithPersonListSortOption(it, context) }
    }
    ...
    override fun handleClickEntry(entry: ClazzWorkSubmissionWithPerson) {
        /* TODO: Add code to go to the appropriate detail view or make a selection
        when(mListMode) {
            ListViewMode.PICKER -> view.finishWithResult(listOf(entry))
            ListViewMode.BROWSER -> systemImpl.go(ClazzWorkSubmissionWithPersonDetailView.VIEW_NAME,
    ...
    override fun handleClickCreateNewFab() {
        ...
        systemImpl.go(ClazzWorkSubmissionWithPersonEditView.VIEW_NAME, mapOf(), context)
         */
    }

    override fun handleClickSortOrder(sortOption: MessageIdOption) {
        val sortOrder = (sortOption as? ClazzWorkSubmissionWithPersonListSortOption)?.sortOrder ?: return
