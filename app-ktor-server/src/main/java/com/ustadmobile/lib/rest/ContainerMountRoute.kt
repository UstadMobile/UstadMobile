...
fun Route.ContainerMountRoute(db: UmAppDatabase) {
    ...
    route("ContainerMount"){
        ...
        head("/{containerUid}/{paths...}"){
            mountContainerWithEntryPaths(db,call, true)
        }


        get("/{containerUid}/{paths...}") {
            mountContainerWithEntryPaths(db,call, false)
        }

    }
}

suspend fun mountContainerWithEntryPaths(db: UmAppDatabase, call: ApplicationCall, isHeadRequest: Boolean){
    val containerUid = call.parameters["containerUid"]?.toLong() ?: 0L
