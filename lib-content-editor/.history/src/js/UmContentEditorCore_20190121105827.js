...
UmContentEditorCore.setFocusToNextUnprotectedFocusableElement = (currentNode, event) => {
    ...
    if(!eventOcurredAfterSelectionProtectionCheck && ($(currentNode).is(labelSelector) || $(currentNode).is(questionActionHolderSelector))){
        ...
        //for add choice button, wait for choice to be added then focus choice body
        setTimeout(function(){
        ...
        setTimeout(function(){
            event.stopImmediatePropagation();
            nextElementFocusSelector = ".question-choice";
            const foundChoices = $(currentNode).find(nextElementFocusSelector);
            if(foundChoices.length > 0){
                nextElementFocusSelector = "p:first-of-type";
                currentNode = $($(foundChoices).last()).get(0);
                console.log("umEditor choice",currentNode);
            }else{
                nextElementFocusSelector = ".question-body + p:first-of-type";
                console.log("umEditor ques",currentNode);
            }
            nextFocusableElement = $(currentNode).find(nextElementFocusSelector);
            console.log("umEditor ques",nextFocusableElement);
