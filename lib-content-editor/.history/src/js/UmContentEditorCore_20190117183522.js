...
UmContentEditorCore.prototype.isElementProtected = (currentNode = null,selectedContentLength = null) =>{
    if(currentNode === null){
    ...
    const preventSelection = selectedContentLength > 0 && $(currentNode).find(doNotRemoveSelector).length > 0;
    const matchesSelector = $(currentNode).is(doNotRemoveSelector);
    return preventSelection || (cursorPositionIndex === 0 && matchesSelector);
}
...
UmContentEditorCore.prototype.onProtectedElementCheck = () => {
    const allowDeletion = matchesSelector && cursorPositionIndex === 0 || preventSelection;
    try{
