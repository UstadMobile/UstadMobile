...
UmContentEditorCore.prototype.saveEditorState = () => {
    ...
    elementToFocusAfterRollbackAction = {
        element: currentActiveElementTracker[0],
        parent: $(currentActiveElementTracker[0]).parent()
    }
};
                ...
                if(UmContentEditorCore.prototype.rollbackChangesAfterMutation(mutations)){
                    ...
                    setTimeout(() => {
                        ...
                        if(elementToFocus.length === 1){
                            UmContentEditorCore.prototype.setCursorToAnyNonProtectedFucusableElement(elementToFocus.get(0))
                        }else{
                            elementToFocus.each((index,element) => {
                                if( === ){
                                    console.log("roll element found");
                                }else{
                                    console.log("roll element not found");
                                }
                                console.log("roll",elementToFocusAfterRollbackAction.element.innerHtml,element.innerHtml);
                            });
                        }
                    }, longerEventTimeout);
                }else{
