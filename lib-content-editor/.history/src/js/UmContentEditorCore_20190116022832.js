...
UmContentEditorCore.prototype.handleCursorPositionAfterUndo = (prevState)=>{
    ...
    switch(prevState.elementPosition){
        case moveCursorToExtraWidgetBelowQuestion:
            elementToFocus = prevState.elementId ? $(editor.find("#"+prevState.elementId)).next()
            :$($($(editor).children()).first().children()).first();
        break;
        case moveCursorToFirstExtraWidget:
            elementToFocus = $($($(editor).children()).first().children()).first();
        break;
        case moveCursorToQuestionBody:
            elementToFocus = $($(editor.find("#"+prevState.elementId)).find(".question-body").children()).first();
        break;
        case 0:
    ...
    if(prevState.elementPosition !== 0){
        ...
        UmContentEditorCore.prototype.scrollToElement(elementToFocus.get(0));
    }
