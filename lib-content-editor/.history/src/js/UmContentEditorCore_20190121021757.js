...
UmContentEditorCore.setFocusToNextUnprotectedFocusableElement = (currentNode) => {
    ...
    if(!eventOcurredAfterSelectionProtectionCheck && ($(currentNode).is(labelSelector) || $(currentNode).is(questionActionHolderSelector))){
        ...
        console.log("umEditor add choice button");
        setTimeout(function(){
            if(UmQuestionWidget.isChoiceAdded ){
                nextElementFocusSelector = ".question-choice-body" + " p:first-of-type";
                currentNode = $($(currentNode).closest("div div.question")).find(".question-choice").last();
                nextFocusableElement = $(currentNode).find(nextElementFocusSelector).get(0);
                console.log("umEditor add choice div",nextFocusableElement);
            }
        },averageEventTimeout);
    }else if(isAddChoiceDiv){
        console.log("umEditor add choice div");
    }else if(isDeleteButton){
        console.log("umEditor delete button");
    }else if($(currentNode).is("button.add-choice,., .action-delete-inner")){
