     ...
     $(mutations).each((index,record) =>{
        ...
        if(allowUndo){
            ...
            if($(record.target).is(selector) && !isQuestionWidgetInsert){
                const lastActivePosition = UmContentEditorCore.getEditorPreviousState().activeElement;
...
UmContentEditorCore.prototype.saveEditorState = () =>{
    ...
    const isAtTheTop = $(activeParentNode).closest("div.container-fluid").length === 1;
    const isBelowQuestion = $($(activeParentNode).prev()).is(".question");
    const isInsideQuestion = $(activeParentNode).closest("div.question")
    const isInsideExtraWidget = $(activeParentNode).is(".extra-content");
    const activeElementPosition =  isInsideExtraWidget && isAtTheTop ? 
    1: (isInsideExtraWidget && isBelowQuestion ? 2:"");
    console.log("parent",)
