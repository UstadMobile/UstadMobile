...
UmContentEditorCore.prototype.saveEditorState = () => {
    ...
    editorStateTracker.push(tinymce.activeEditor.selection.getNode());
    const discardToIndex  = editorStateTracker.length - 3;
    editorStateTracker.forEach((element,index) => {
        if(index <= discardToIndex){
            editorStateTracker.splice(index,1);
        }
    });
    console.log("roll back",editorStateTracker)
};
