     ...
     if(isSoftKeyboard){
        $(mutations).each((index,record) =>{
            if(record.removedNodes.length > 0){
                console.log("roll",record.type === "childList" && record.removedNodes.length > 0 && !isQuestionWidgetInsert && !rollbackActionPerformed) 
            && !isDeleteOrCutAction);
            }
            
            if((record.type === "childList" && record.removedNodes.length > 0 && !isQuestionWidgetInsert && !rollbackActionPerformed) 
            && !isDeleteOrCutAction){
                let selector = ".dont-remove, .immutable-content, div[class^='question'], #umEditor";
