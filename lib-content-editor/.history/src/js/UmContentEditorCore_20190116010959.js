...
UmContentEditorCore.prototype.handleProtectedElementsOnSoftKeyboard = (mutations) => {
     ...
     $(mutations).each((index,record) =>{
        ...
        //console.log("datacheck",handleOnParagraph);
        if($(record.previousSibling).is(".immutable-content")){
            const handleOnParagraph =   record.oldValue && record.oldValue.length === 1 && record.target.nodeValue.length === 0;
            const nextFocusable = UmContentEditorCore.prototype.getNextElementMatchingSelector("p")
            console.log("datacheck",nextFocusable);
        }else if(allowUndo){
            console.log("datacheck big picture");
