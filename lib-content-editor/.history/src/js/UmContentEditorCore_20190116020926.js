        ...
        if(allowUndo){
            ...
            if($(record.target).is(selector) && !isQuestionWidgetInsert){
                ...
                setTimeout(() => {
                    ...
                    UmContentEditorCore.prototype.handleCursorPositionAfterUndo()
                }, longerEventTimeout);
            }
        }
    });
...
UmContentEditorCore.prototype.handleCursorPositionAfterUndo = (prevState)=>{
    ...
    switch(prevState.elementPosition){
        case moveCursorToExtraWidgetBelowQuestion:
            elementToFocus = $(editor.find("#"+prevState.elementId)).next();
        break;
        case moveCursorToFirstExtraWidget:
            elementToFocus = $(editor).children().get(0);
        break;
        case moveCursorToQuestionBody:
            elementToFocus = $(editor.find("#"+prevState.elementId)).children().get(0)
        break;
        case 0:
