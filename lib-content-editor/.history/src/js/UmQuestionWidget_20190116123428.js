...
UmQuestionWidget.prototype.startEditing = function() {
    ...
    $(this.element).find(".question-retry-btn").html("<button class='float-right qn-retry .extra-btn {
    ...
    cursor: pointer
}' data-um-preview='support'>"+UmQuestionWidget._locale.placeholders.labelForTryAgainOptionBtn+"</button>");
...
UmFillTheBlanksQuestionWidget.prototype.onQuestionAnswerChecked = (event) =>{
    ...
    const message = userAnswerText.length > 0 ?
        (isCorrectChoice ? correctChoiceText: wrongChoiceText):UmQuestionWidget._locale.placeholders.warningOnSubmitEmptyField;
