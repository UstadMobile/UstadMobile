...
UmContentEditorCore.setFocusToNextUnprotectedFocusableElement = (currentNode,event) => {
    ...
    if(!eventOcurredAfterSelectionProtectionCheck && ($(currentNode).is(labelSelector) || $(currentNode).is(questionActionHolderSelector))){
        ...
        setTimeout(function () {
            ...
            currentNode = isDeletion ? currentNode: (isAddChoiceButton ? $($(currentNode).closest("div div.question")).find(".question-choice").last():$($($(currentNode).closest("div div.question")).next()).find(nextElementFocusSelector));
