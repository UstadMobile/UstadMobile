 ...
 */
UmContentEditorCore.initEditor = (locale = "en",path = "",showToolbar = false) => {
    umConfig = JSON.parse(JSON.stringify(umConfig));
    UmQuestionWidget.loadPlaceholders(umConfig && umConfig.locale ? umConfig.locale:"en"
        ,umConfig && umConfig.test ? umConfig.test:false);
    showToolbar = umConfig && umConfig.toolbar ? umConfig.toolbar:false;
    const configs = {
        selector: '#umEditor',
