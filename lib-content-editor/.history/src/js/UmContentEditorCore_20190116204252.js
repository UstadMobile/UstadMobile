...
UmContentEditorCore.prototype.saveEditorState = () =>{
    ...
    editorStateTracker.push(editorState);
    const lastIndex  = editorStateTracker.length - 3;
    editorStateTracker.forEach((element,index) => {
        if(index <= lastIndex){
            editorStateTracker.splice(index,1);
        }
    });
    previousEditorState = editorStateTracker[0];
    console.log("SavedData",editorStateTracker);
};
...
UmContentEditorCore.initEditor = (locale = "en", showToolbar = false) => {
    ...
    const configs = {
        ...
        setup: (ed) => {
            ...
            ed.on('SaveContent', function name(params) {
        ...
        try{
            ...
            const contentChangeObserver = new MutationObserver(function(mutations) {
                ...
                if(UmContentEditorCore.prototype.rollbackChangesAfterMutation(mutations)){
                    ...
                    performedUndoAction = true;
                    editorContainer.html(previousEditorState.content);
                    setTimeout(() => {
                        performedUndoAction = false;
                        UmContentEditorCore.prototype.handleCursorPositionAfterUndo(previousEditorState.element);
                    }, longerEventTimeout);
                }
