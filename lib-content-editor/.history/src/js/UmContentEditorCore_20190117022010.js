...
UmContentEditorCore.prototype.insertQuestionTemplate = (questionTypeIndex,isTest = false) => {
    ...
    const nextChoiceId = UmQuestionWidget.ELEMENT_ID_TAG+UmQuestionWidget.getNextUniqueId();
    ...
    let rollbackChanges = false;
     if(isSoftKeyboard){
        $(mutations).each((index,record) =>{
            ...
            && !isDeleteOrCutAction){
                let selector = ".dont-remove, .immutable-content, rollbackChangesAfterMutation, div[class*='question'], #umEditor";
