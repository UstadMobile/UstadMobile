...
UmContentEditorCore.prototype.rollbackChangesAfterMutation = (mutations) => {
    let rollbackAfterMutation = false;
     if(isSoftKeyboard){
        $(mutations).each((index,record) =>{
            if((record.type === "childList" && record.removedNodes.length > 0 && !isQuestionWidgetInsert && !performedUndoAction) 
            && !isDeleteOrCutAction){
...
UmContentEditorCore.prototype.saveEditorState = () =>{
    ...
    console.log("states",tinymce.activeEditor.selection.getNode());
};
...
UmContentEditorCore.initEditor = (locale = "en", showToolbar = false) => {
    ...
    const configs = {
        ...
        init_instance_callback: (ed) => {
             ...
             */
            ed.on('keyup', () => {
                console.log("SaveData - Update Ui (KeyUp)")
             ...
             */
            ed.on('keydown', (e) => {
                console.log("SaveData - Saved (KeyDown)")
    ...
    tinymce.init(configs).then(() => {
        ...
        try{
            ...
            const contentChangeObserver = new MutationObserver(function(mutations) {
                console.log("SaveData - Change Mutation")
                ...
                if(UmContentEditorCore.prototype.rollbackAfterMutation(mutations)){
                    //wait for mutation event to finish then rollback changes.
                    console.log("prevState",editorStateTracker)
                    $("#umEditor").html(editorStateTracker.content);
                    performedUndoAction = true;
                    setTimeout(() => {
