...
/** Selector to prevent from deleting protected elements */
...
UmContentEditorCore.prototype.isElementProtected = (currentNode = null,selectedContentLength = null, isDeleteKey = false, isCallback) =>{
    if(currentNode === null){
        currentNode = tinymce.activeEditor.getNode();
    }

    if(selectedContentLength === null){
        selectedContentLength = this.selectNodeContents.length;
    }
    const cursorPositionIndex = UmContentEditorCore.prototype.getCursorPositionRelativeToTheEditableElementContent();
    const preventSelection = selectedContentLength > 0 && $(currentNode).find(doNotRemoveSelector).length > 0;
    const matchesSelector = $(currentNode).is(doNotRemoveSelector);
    return isCallback ? (preventSelection || (cursorPositionIndex === 0 && matchesSelector)) : ((isDeleteKey && matchesSelector && cursorPositionIndex === 0) || preventSelection);
}
