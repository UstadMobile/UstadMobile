...
UmContentEditorCore.prototype.handleProtectedElementsOnSoftKeyboard = (mutations) => {
     ...
     $(mutations).each((index,record) =>{
        ...
        const handleOnParagraph =  $(record.previousSibling).is(".immutable-content") && record.oldValue && record.oldValue.length === 1 && record.target.nodeValue.length === 0;
        if(handleOnParagraph){
            console.log("datacheck",record);
        }else if(allowUndo){
            ...
            if($(record.target).is(selector) && !isQuestionWidgetInsert){
                ...
                tinymce.activeEditor.selection.select(activeElement, true);
                performedUndoAction = true;
                setTimeout(() => {
...
UmContentEditorCore.prototype.saveEditorState = () =>{
    ...
    editorStateTracker.push({activeElement:activeElement,content:editorState});
...
UmContentEditorCore.saveSelection = ()=>{
    if (savedSel) {
...
UmContentEditorCore.retrieveSelection = ()=>{
    if (savedSel) {
    ...
    //const tempNode =  tinymce.html.DomParser().parse($(savedSelActiveElement).html());
    tinymce.activeEditor.selection.select(savedSelActiveElement, true);
    }catch(e){
