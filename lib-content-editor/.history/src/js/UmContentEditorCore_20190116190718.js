 ...
 */
UmContentEditorCore.prototype.preventMutationOnSoftKeyboardEvents = (mutations) => {
    let rollbackAfterMutation = false;
     if(isSoftKeyboard){
        $(mutations).each((index,record) =>{
            if((record.type === "childList" && record.removedNodes.length > 0 && !isQuestionWidgetInsert && !performedUndoAction) && !isDeleteOrCutAction){
