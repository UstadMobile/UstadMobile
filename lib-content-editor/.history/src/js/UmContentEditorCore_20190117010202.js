...
UmContentEditorCore.prototype.saveEditorState = () => {
    previousEditorState = tinymce.activeEditor.getContent();

    //track element to be focused
    currentActiveElementTracker.push(tinymce.activeEditor.selection.getNode());
    const discardToIndex  = currentActiveElementTracker.length - 3;
    currentActiveElementTracker.forEach((element,index) => {
        if(index <= discardToIndex){
            currentActiveElementTracker.splice(index,1);
        }
    });
    console.log("roll back",currentActiveElementTracker)
};
        ...
        try{
            ...
            const contentChangeObserver = new MutationObserver(function(mutations) {
                ...
                if(UmContentEditorCore.prototype.rollbackChangesAfterMutation(mutations)){
                    performedUndoAction = true;
                    console.log("roll-active","rolling back")
                    editorContainer.html(previousEditorState);
                    setTimeout(() => {
                        performedUndoAction = false;
                        //UmContentEditorCore.prototype.handleCursorPositionAfterUndo(previousEditorState.activeElement);
                    }, longerEventTimeout);
                }else{
                    UmContentEditorCore.prototype.saveEditorState();
                }
