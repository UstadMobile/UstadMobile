...
UmContentEditorCore.setFocusToNextUnprotectedFocusableElement = (currentNode,event) => {
    ...
    if(!eventOcurredAfterSelectionProtectionCheck && ($(currentNode).is(labelSelector) || $(currentNode).is(questionActionHolderSelector))){
    ...
    }else if($(currentNode).is(".question-add-choice")){
        ...
        nextFocusableElement = $($($(currentNode).closest("div div.question")).next()).find(nextElementFocusSelector).get(0);
        UmContentEditorCore.prototype.setCursorToAnyNonProtectedFucusableElement(nextFocusableElement);
        UmContentEditorCore.prototype.scrollToElement(nextFocusableElement);
        ...
        currentNode =  isDeletion ? $($(currentNode).closest("span")).closest("div div.question"): currentNode;
        //Wait for the action to take place i.e delete/add then scroll to the respective element.
        setTimeout(function () {
            nextElementFocusSelector = (isDeletion ? ".question-body":(isAddChoiceDiv ? "p:first-of-type":".question-choice-body") + " p:first-of-type");
            currentNode = isDeletion ? currentNode: (isAddChoiceButton ? $($(currentNode).closest("div div.question")).find(".question-choice").last():
            $($($(currentNode).closest("div div.question")).next()).find(nextElementFocusSelector));
            nextFocusableElement = $(currentNode).find(nextElementFocusSelector).get(0);
            UmContentEditorCore.prototype.setCursorToAnyNonProtectedFucusableElement(nextFocusableElement);
            UmContentEditorCore.prototype.scrollToElement(nextFocusableElement);
            return nextFocusableElement;
        },isAddChoiceButton ? 0:averageEventTimeout);
