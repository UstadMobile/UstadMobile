...
UmContentEditorCore.setKeyboardType = (physicalKeyboardAttached) =>{
    isSoftKeyboard = !physicalKeyboardAttached;
}

UmContentEditorCore.setDefaultLanguage = (locale)=>{
    UmQuestionWidget.loadPlaceholders(locale,false);
}

/**
 * Initialize tinymce editor to the document element
 * @param locale => Default UMEditor language locale
 * @param isTest => path resolver for test
 * @param showToolbar => Flag to show and hide default tinymce toolbar
 */
UmContentEditorCore.initEditor = (locale = "en", isTest = false, showToolbar = false) => {
    ...
    const configs = {
        ...
        init_instance_callback: (ed) => {
             ...
             */
            ed.on('keydown', (e) => {
                isSoftKeyboard =  isSoftKeyboard && e.keyCode===299;
                if(!isSoftKeyboard){
